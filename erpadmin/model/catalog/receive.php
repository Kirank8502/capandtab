...
class Receive extends \Opencart\System\Engine\Model {
	public function addReceive($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "receive SET `product_id` = '" . $this->db->escape($product_ids) . "', file = '".(!empty($data['filename']) ? $this->db->escape($data['filename']) : '')."' , vendor_id = '" . (!empty($data['vendor_id']) ? $this->db->escape($data['vendor_id']) : '') . "', orders_id = '" . (!empty($data['orders_id']) ? $this->db->escape($data['orders_id']) : 0) . "', qty = '" . (!empty($data['qty']) ? (int)$data['qty'] : 0) . "', kgs_qty = '" . (!empty($data['kgs_qty']) ? (int)$data['kgs_qty'] : 0) . "', receive_date = date('" . $data['receive_date'] . "'), date_added = NOW()");

		$receive_id = $this->db->getLastId();

		return true;
		
		$this->cache->delete('receive');
	}
	
	public function editReceive($receive_id, $data) {

		$product_ids = isset($data['product_id']) ? (is_array($data['product_id']) ? (implode(',', (!empty($data['product_id']) ? $data['product_id'] : 0))) : 0):0;

      	$this->db->query("UPDATE " . DB_PREFIX . "receive SET `product_id` = '" . $this->db->escape($product_ids) . "', file = '".(!empty($data['filename']) ? $this->db->escape($data['filename']) : '')."' , vendor_id = '" . (!empty($data['vendor_id']) ? $this->db->escape($data['vendor_id']) : '') . "', orders_id = '" . (!empty($data['orders_id']) ? $this->db->escape($data['orders_id']) : 0) . "', qty = '" . (!empty($data['qty']) ? (int)$data['qty'] : 0) . "', kgs_qty = '" . (!empty($data['kgs_qty']) ? (int)$data['kgs_qty'] : 0) . "', receive_date = date('" . $data['receive_date'] . "') WHERE `receive_id` = ".$receive_id);
	...
	public function deleteReceive($receive_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "receive WHERE receive_id= '" . (int)$receive_id. "'");			
		$this->cache->delete('order');
	}	
	
	public function getReceive($receive_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "receive WHERE receive_id= '" . (int)$receive_id. "'");
		
		return $query->row;
	}

	public function getOrderDetail($receive_id) {
		$query = $this->db->query("SELECT od.*,o.* FROM " . DB_PREFIX . "receive o LEFT JOIN " . DB_PREFIX . "order_details od ON o.receive_id = od.receive_id  WHERE o.receive_id= '" . (int)$receive_id. "'");
	...
	public function getReceives($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "receive WHERE 1";
		
		$sql .= " ORDER BY `receive_id`";	
	...
	public function getTotalReceives() {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "receive");
	...
	public function getOrderDetails($data=array()) {
		...
		$sql = "SELECT `status`, COUNT(receive_id) AS total FROM ".DB_PREFIX."receive WHERE 1 ";
	...
	public function getOrderId() {
		$query = $this->db->query("SELECT MAX(po_no) AS latest_order_id FROM " . DB_PREFIX . "receive");
