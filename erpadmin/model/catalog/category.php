...
class Category extends \Opencart\System\Engine\Model {
	...
	public function addCategory(array $data): int {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "category` SET `image` = '" . (!empty($data['image']) ? $this->db->escape((string)$data['image']) : '') . "', `parent_id` = '0', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (!empty($data['column']) ? (int)$data['column'] : 0) . "', `sort_order` = '0', `status` = '1', `date_modified` = NOW(), `date_added` = NOW()");

		$category_id = $this->db->getLastId();

		// foreach ($data['category_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "category_description` SET `category_id` = '" . (int)$category_id . "', `language_id` = '1', `name` = '" . $this->db->escape($data['category_name']) . "', `description` = 'No Data', `meta_title` = 'No Data', `meta_description` = 'No Data', `meta_keyword` = 'No Data'");
		// }
	...
	public function editCategory(int $category_id, array $data): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "category` SET SET `image` = '" . (!empty($data['image']) ? $this->db->escape((string)$data['image']) : '') . "', `parent_id` = '0', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (!empty($data['column']) ? (int)$data['column'] : 0) . "', `sort_order` = '0', `status` = '1', `date_modified` = NOW() WHERE `category_id` = '" . (int)$category_id . "'");

		$this->db->query("DELETE FROM `" . DB_PREFIX . "category_description` WHERE `category_id` = '" . (int)$category_id . "'");

		// foreach ($data['category_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "category_description` SET `category_id` = '" . (int)$category_id . "', `language_id` = '" . 0 . "', `name` = '" . $this->db->escape($data['category_name']) . "', `description` = 'No Data', `meta_title` = 'No Data', `meta_description` = 'No Data', `meta_keyword` = 'No Data'");
		// }
	...
	public function getCategories(array $data = []): array {
		$sql = "SELECT c1.`category_id` AS `category_id`, cd1.`name` AS `name`, c1.`parent_id`, c1.`sort_order`, c1.`status` FROM `" . DB_PREFIX . "category` c1 LEFT JOIN `" . DB_PREFIX . "category_description` cd1 ON (c1.`category_id` = cd1.`category_id`) WHERE cd1.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";

		if (!empty($data['filter_name'])) {
			$sql .= " AND cd1.`name` LIKE '" . $this->db->escape((string)$data['filter_name']) . "'";
		}
