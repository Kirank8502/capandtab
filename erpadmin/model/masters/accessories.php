...
class Accessories extends \Opencart\System\Engine\Model {
	...
	public function editAccessory($accessories_id, $data) {
      	$this->db->query("UPDATE " . DB_PREFIX . "accessories SET `name` = '" . $this->db->escape($data['accessories_name']) . "', qty = '" . (int)$data['accessories_qty']."', status = '" . (int)$data['accessories_status'] ."' WHERE accessories_id= '" . (int)$accessories_id. "'");
