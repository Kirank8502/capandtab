 ...
 */
class Product1 extends \Opencart\System\Engine\Controller {
	/**
	 * @return void
	 */
	public function index(): void {
		...
		$this->response->setOutput($this->load->view('catalog/product_1', $data));
	}
	...
	protected function getList(): string {
		...
		$data['action'] = $this->url->link('catalog/product_1.list', 'user_token=' . $this->session->data['user_token'] . $url);
		...
		$this->load->model('catalog/product_1');

		$this->load->model('tool/image');

		$product_total = $this->model_catalog_product1->getTotalProducts($filter_data);

		$results = $this->model_catalog_product1->getProducts($filter_data);
		...
		foreach ($results as $result) {
			...
			$product_specials = $this->model_catalog_product1->getSpecials($result['product_id']);
			...
			$data['products'][] = [
				...
				'edit'       => $this->url->link('catalog/product_1.form', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $result['product_id'] . ($result['master_id'] ? '&master_id=' . $result['master_id'] : '') . $url),
				'variant'    => (!$result['master_id'] ? $this->url->link('catalog/product_1.form', 'user_token=' . $this->session->data['user_token'] . '&master_id=' . $result['product_id'] . $url) : '')
			];
		}
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $this->config->get('config_pagination_admin'),
			'url'   => $this->url->link('catalog/product_1.list', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}')
		]);
		...
		return $this->load->view('catalog/product_list_1', $data);
	}
	...
	public function form(): void {
		...
		if (isset($this->request->get['master_id'])) {
			...
			$url = $this->url->link('catalog/product_1.form', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $this->request->get['master_id']);
		...
		$data['breadcrumbs'][] = [
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('catalog/product_1', 'user_token=' . $this->session->data['user_token'] . $url)
		];
		...
		$data['save'] = $this->url->link('catalog/product_1.save', 'user_token=' . $this->session->data['user_token']);
		$data['back'] = $this->url->link('catalog/product_1', 'user_token=' . $this->session->data['user_token'] . $url);
		...
		if ($product_id) {
			...
			$product_info = $this->model_catalog_product1->getProduct($product_id);
		}
		...
		$this->response->setOutput($this->load->view('catalog/product_form_1', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'catalog/product1')) {
	...
	public function delete(): void {
		...
		if (!$json) {
			$this->load->model('catalog/model_catalog_product1');
